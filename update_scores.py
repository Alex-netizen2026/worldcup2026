...
def write_scores(path, data):
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        for match_no in sorted(data, key=lambda x: int(x)):
            writer.writerow({"MatchNo": match_no, "Score": data[match_no]})
...
def main():
    ...
    current = read_scores(SCORES_FILE)
    manual = read_scores(MANUAL_FILE)

    updated = 0

    for match_no, new_score in manual.items():
        old_score = current.get(match_no, "")
        if old_score != new_score:
            current[match_no] = new_score
            updated += 1
    ...
    write_scores(SCORES_FILE, current)
